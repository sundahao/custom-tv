    ...
    private void arrayTowords() {
        ...
        for (int i = 0; i < array.length; i++) {
            ...
            if (array[i] >= 0 && array[i] < 0x7f) {
                ...
                try {
                    if (i < array.length - 1) {
                        ub = Character.UnicodeBlock.of((array[i + 1]));
                    }
                } catch (Exception e) {
                    e.printStackTrace();

                }
                if (ub == Character.UnicodeBlock.GENERAL_PUNCTUATION || ub == Character.UnicodeBlock.CJK_SYMBOLS_AND_PUNCTUATION || ub == Character.UnicodeBlock.CJK_SYMBOLS_AND_PUNCTUATION || ub == Character.UnicodeBlock.HALFWIDTH_AND_FULLWIDTH_FORMS || ub == Character.UnicodeBlock.CJK_COMPATIBILITY_FORMS) {
                    continue;
                }
                j++;
            }
            ...
            if (linewidth + measureText >= textWidth) {
                ...
                for (int k = point; k < i; k++) {
                    ...
                    if (String.valueOf(words[k]).equals("\n")) {
                        ...
                        canvas.drawText(words[k], widthPoint + getPaddingLeft(), (float) (mPaint.getTextSize() + lineSpacing) * (line + 1) + getPaddingTop(), mPaint);
                    }
                    widthPoint = widthPoint + mPaint.measureText(words[k]) + ((textWidth - linewidth) / (i - point - 1));
                    // 如果不是中文,增加一个空格
